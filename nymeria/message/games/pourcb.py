...
class Pourcb:
    ...
    def __init__(self, message):
        ...
        self.number = (
            int(commande[2]) if len(commande) >= 3 and commande[2].isdigit() else 10
        )
    ...
    async def launch(self, client):
        ...
        await self.author.send(
            f"Donne un nombre entre 1 et {self.number}"
        )  # mention2->author

        def is_correct1(m):
            return (
                m.author == self.mention1
        ...
        if rep1 == rep2:
            await self.channel.send(
                f"{self.mention1.mention} {self.author.mention}\nGagné, vous avez tous les deux choisi {rep1}"
            )
        else:
            await self.channel.send(
                f"Perdu, {self.mention1.mention} a choisi {rep1} et {self.author.mention} a choisi {rep2}"
            )
