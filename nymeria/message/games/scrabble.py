...
class Scrabble:
    ...
    async def launch(self, client):
        ...
        while longueur == 0:
            try:
                ...
                await self.channel.send(
                    f"**{self.author.name}**, entre une longueur de mot entre 1 et "
                    + str(lmax)
                    + ":"
                )
        ...
        await self.channel.send(
            f"**{self.author.name}**, ton anagramme: " + mot_melange
        )
        ...
        # à cette anagramme
        if (
            sous_ensemble(sol, mot)
            and sol_t in minf_idx
            and recherche(sol, minf_idx[sol_t])
        ):
            if len(sol) == len(mot):
                # Si la solution proposée et le mot à deviner
                # On la même longueur: champion!
                await self.channel.send(
                    f"**{self.author.name}**, bien joué, c'est une solution exacte !"
                )
            else:
                # Sinon, on a une solution partielle
                # On affiche les meilleures réponses possible
                await self.channel.send(
                    f"**{self.author.name}**, c'est une solution partielle"
                )
        ...
        await self.channel.send(
            f"Mauvaise réponse, la meilleure réponse possible était {mot}"
        )
